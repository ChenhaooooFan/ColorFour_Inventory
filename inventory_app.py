...
st.title("ColorFour Inventory 系统（升级：bundle 断裂缝合 + 1–4件通吃）")
...
def parse_pdf_with_tokens(pf):
    ...
    返回： sku_counts(dict)，item_quantity(或 "")
    """
    item_q = ""
    try:
...
if selected_pdfs and csv_file:
    ...
    # —— 每个 PDF：读取标注值、提取 SKU（新解析器），另外专项扫描 NM001 / Holiday Bunny（仅用于对账说明）——
    pdf_item_list = []
    pdf_sku_counts = {}
    pdf_nm001_counts = {}
    pdf_hb_counts = {}
    ...
    for pf in selected_pdfs:
        # A) 统一用“token + 兜底”的新解析器
        sku_counts_single, qty_val = parse_pdf_with_tokens(pf)

        # B) NM001 / Holiday Bunny 扫描（仅用于说明，不参与扣减）
        nm001_qty_scan = 0
        hb_qty_scan = 0
        with pdfplumber.open(pf) as pdf:
            for page in pdf.pages:
                lines = (_norm_text(page.extract_text()) or "").split("\n")
                for line in lines:
